...
Ces 4 mots hexadécimaux sont pensés pour être utilisés, par exemple, dans une fonction de type `matrixWrite()` côté microcontrôleur (Arduino, STM32, etc.).
...
- `row` = numéro de ligne (0…7)
- `col` = numéro de colonne (0…12)
...
Le champ texte utilise une police à chasse fixe (monospace) pour faciliter la lecture et l’alignement des valeurs.
...
> Vous pouvez ajouter ici une capture d’écran de l’application montrant la grille et les valeurs hexadécimales générées.

Par exemple :

- Grille de LED avec un motif dessiné (cœur, lettre, etc.).
- Zone de texte en dessous avec les 4 mots hexadécimaux.
